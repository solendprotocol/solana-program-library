    ...
    match instruction {
        ...
        WrapperInstruction::DepositMax => {
            ...
            let remaining_deposit_capacity = Decimal::from(reserve.config.deposit_limit)
                .try_sub(reserve.liquidity.total_supply()?)?;
            let source_liquidity_balance = std::cmp::min(
                source_liquidity_balance,
                remaining_deposit_capacity.try_floor_u64()?,
            );
        ...
        WrapperInstruction::WithdrawExact { liquidity_amount } => {
            msg!("Instruction: WithdrawExact");
            ...
            let liquidity_out = reserve
                .collateral_exchange_rate()?
                .collateral_to_liquidity(ctoken_amount)?;
            if liquidity_out < liquidity_amount {
                ctoken_amount += 1;
            }
            ...
            let instruction = withdraw_obligation_collateral_and_redeem_reserve_collateral(
                *solend_program_id.key,
                ctoken_amount,
                *reserve_collateral_info.key,
                *user_collateral_info.key,
                *reserve_info.key,
                *obligation_info.key,
                *lending_market_info.key,
                *user_liquidity_info.key,
                *reserve_collateral_mint_info.key,
                *reserve_liquidity_supply_info.key,
                *obligation_owner_info.key,
                *user_transfer_authority_info.key,
